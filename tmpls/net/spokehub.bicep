...
// DNS Forwarder in Hub ============================================================
var nicNameDnsForwarderHub = format('nicDnsForwarderHub{0}', networkAddrB)
resource networkInterfaceDnsForwarderHub 'Microsoft.Network/networkInterfaces@2020-11-01' = {
  name: nicNameDnsForwarderHub
  location: location
  properties: {
    ipConfigurations: [
      {
        name: format('ipconfig-dnsfowarder-hub{0}', networkAddrB)
        properties: {
          privateIPAllocationMethod: 'Dynamic'
          subnet: {
            id: subnetAzureHubDefault.id
...
var vmNameDnsForwarderHub = format('dnsfowarderhub{0}', networkAddrB)
resource vmDnsForwarderHub 'Microsoft.Compute/virtualMachines@2020-12-01' = {
  name: vmNameDnsForwarderHub
  location: location
  properties: {
    hardwareProfile: {
      vmSize: 'Standard_D2s_v3'
    ...
    osProfile: {
      computerName: vmNameDnsForwarderHub
      adminUsername: adminUserName
      linuxConfiguration: {
        disablePasswordAuthentication: true
        ssh: {
    ...
    networkProfile: {
      networkInterfaces: [
        {
          id: networkInterfaceDnsForwarderHub.id
        }
      ]
    }
  }
...
resource extensionBaseDnsForwarderHub 'Microsoft.Compute/virtualMachines/extensions@2019-12-01' = {
  name: format('{0}/extensionBase', vmDnsForwarderHub.name)
  location: location
  properties: {
    publisher: 'Microsoft.Azure.Extensions'
    type: 'CustomScript'
    ...
    settings: {
      fileUris: [
        'https://raw.githubusercontent.com/shoshii/armtemp/master/tmpls/bin/forwarderSetup.sh'
      ]
      commandToExecute: format('sh forwarderSetup.sh 168.63.129.16 {0}', virtualNetworkAzureHub.properties.addressSpace.addressPrefixes)
    }
  }
}
